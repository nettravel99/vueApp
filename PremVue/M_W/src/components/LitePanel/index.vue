...
export default {
    ...
    computed: {
        src() {
            if (this.LiteType === 'T') {
                return `transwrapper^mwdn?SES=${this.$store.state.user.token}&TR=${this.formatTrans(this.LiteValue)}&`
            } else if (this.LiteType === 'TASK') {
                return `taskwrapper^mwdn?SES=${this.$store.state.user.token}&TASK=${this.LiteValue}&`
            } else if (this.LiteType === 'DEMO') {
                return `demowrapper^mwdn?SES=${this.$store.state.user.token}&K=${this.LiteValue}&`
            } else if (this.LiteType === 'DENIAL') {
                return `premier/denial/denialwrapper^mwdn?SES=${this.$store.state.user.token}&TR=${this.LiteValue}&`
            }else {
                return '';
            }
        }
