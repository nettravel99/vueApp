<template>
  <div>
    ...
    <el-dialog :title="'Diagnosis Codes Lookup'" :visible.sync="dialogFormVisible" append-to-body>
      <el-row>
        <el-col :span="24">
      ...
      <el-row>
        <el-col :span="24">
          <hot-table
            ...
            :key="'hotTableComponentDiagnonsis'"
      ...
      <el-row>
        <el-col :span="24">
          <el-button type="primary" icon="el-icon-caret-left" @click="expandDiangnosis('L')" />
          <el-button type="primary" icon="el-icon-caret-right" @click="expandDiangnosis('R')" />
        </el-col>
      </el-row>
...
export default {
    ...
    return {
      ...
      afterOnCellMouseDown: async function(event, coords, td) {
        ...
        if (
          ...
          _vm.hvalue = data[0]
          await _vm.expandDiangnosis('R')
          return
        }
        _vm.lkvalue = data[0]
        if (await _vm.checkValid()) _vm.confirmLookup()
      }
    }
  },
  computed: {
  ...
  watch: {
    ...
    lkvalue(val, oval) {
      if ((val && val.indexOf('_') > -1) || (val && val.indexOf('*') > -1) || (val && val.indexOf('-') > -1)) {
        this.hvalue = val
        this.lkvalue = oval
      }
    }
  ...
  methods: {
    ...
    async expandDiangnosis(D) {
      const data = await M('expandDiangnosis^MWLK', {
        search: this.hvalue,
        asearch: this.lkvalue,
        dir: D
      })
      if (data && data.data && data.data.searchValue) {
        this.lkvalue = data.data.searchValue
    ...
    async initLookup() {
      const data = await M('diagnosis^MWLK', {
        lookup: this.lookup
      })
