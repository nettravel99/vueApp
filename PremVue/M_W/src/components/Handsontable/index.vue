<template>
<el-card v-if="rerender" class="box-card">
    <el-button type="info" size="mini" round>
        <el-tooltip content="Download Excel File" placement="top" effect="light">
            <a :href="'excel^mwr?SES='+$store.state.user.token+'&EXT=CSV&ID='+ID">
        <b>
    ...
    <el-tooltip content="Customize columns and create sets" placement="top" effect="light">
        <el-button type="primary" size="mini" icon="el-icon-edit" circle @click="saveSetDialogVisible = true" />
    </el-tooltip>
    <el-tooltip content="Clear current set and load default set" placement="top" effect="light">
        <el-button type="primary" size="mini" icon="el-icon-close" circle @click="handleTableClear"></el-button>
    </el-tooltip>
    <el-dialog title="Edit Columns" :visible.sync="saveSetDialogVisible" width="40%">
       <el-transfer style="margin:0 0;" v-model="selectedHeaderSet" :data="headerSelect" filterable :titles="['Available', 'Set']" target-order="push" />
        <br><br>
        <span><b>Set:</b></span><br><br>
        <small>If Selecting a new set, please make sure the newly typed set is selected in the box below before saving</small>
        <el-select v-model="setName" size="mini" style="width:100%" filterable allow-create clearable placeholder="Choose set or type a new one">
            <el-option v-for="item in sets" :key="item.name" :label="item.name" :value="item.name" />
        </el-select>
        <span slot="footer" class="dialog-footer">
        <el-button @click="handleCancel">Cancel</el-button>
        <el-button type="danger" @click="handleDeleteSet" :disabled="!setName">Delete</el-button>
        <el-button type="primary" @click="handleSaveSet" :disabled="selectedHeaderSet && selectedHeaderSet.length < 1">Save/Load</el-button>
      </span>
    </el-dialog>
    <el-radio-group v-model="selectedSet" @change="handleLoadSet">
        <el-radio v-for="item in sets" :key="item.name" :label="item.name"><b>{{ item.name }}</b>&nbsp;&nbsp;&nbsp;</el-radio>
    </el-radio-group>
    ...
    <hot-table :id="'CSVTableID-' + ID" :key="'CSVTableRoot-' + ID+ '-'+tkey" :ref="'hotTableComponent'" :root="'CSVTableRoot-' + ID" :data="data" :col-headers="headers" :row-headers="true" :read-only="true" :column-sorting="true" :manual-column-freeze="true" :context-menu="contextMenu" :filters="true" :dropdown-menu="['filter_by_condition','filter_operators','filter_by_condition2','filter_by_value','filter_action_bar']" :height="tableHeight" :license-key="'a159c-435af-b9fe5-74b04-7e35f'" />
...
export default {
    ...
    data: function () {
        ...
        return {
            ...
            reportStatus: null,
            loading: true,
            tableHeight: window.innerHeight - 182,
                ...
                items: {
                    GoToDemographics: {
                        ...
                        name: function () {
                            ...
                            return 'Demographics by Acct#' // Name can contain HTML
                        },
                        callback: function () {
                            // Callback for specific option
                            const hot = this
                            setTimeout(function () {
                                var val = hot.getDataAtCell(
                                    hot.getSelectedLast()[0],
                                    hot.getSelectedLast()[1]
                                )
                                _vm.showDemo(val)
                                // window.open("http://google.com/?" + val, "_blank");
                            }, 0)
                        }
                    },
                    GoToAccountInquery: {
                        ...
                        name: function () {
                            // `name` can be a string or a function
                            return 'Account Inquery by Tr#' // Name can contain HTML
                        },
                        callback: function () {
                            // Callback for specific option
                            const hot = this
                        ...
                        name: function () {
                            // `name` can be a string or a function
                            return 'New Task by Tr#' // Name can contain HTML
                        },
                        callback: function () {
                            // Callback for specific option
                            const hot = this
                    ...
                    GoToDenials: {
                        ...
                        name: function () {
                            // `name` can be a string or a function
                            return 'Denials by Tr#' // Name can contain HTML
                        },
                        callback: function () {
                            // Callback for specific option
                            const hot = this
    ...
    async mounted() {
        await this.handleMounted()
        let vm = this;
        setTimeout(function(){ vm.$refs.hotTableComponent.hotInstance.render(); },1000);

    },
    methods: {
        handleCancel() {
            setTimeout(() => {
                this.selectedHeaderSet = []
            }, 1000)
            this.$nextTick(() => {
                this.saveSetDialogVisible = false
            })
        },
        async handleTableClear() {
            this.selectedSet = '';
            this.setName = ''
            await this.handleLoadSet(true,true);
            setTimeout(() => {
                this.selectedHeaderSet = []
            }, 1000) 
            this.$nextTick(() => {
        ...
        async handleDeleteSet() {
            const data = await M('deleteHeaderSet^MWR', {
                name: this.setName,
                hdr: this.selectedHeaderSet,
                id: this.ID
            })
            let status
        ...
        async handleLoadSet(noMsg,clear) {
            const data = await M('loadHeaderSet^MWR', {
            ...
            this.setLoaded = true;
            if (clear) this.setLoaded = false;
            await this.handleMounted()
        },

        async handleSaveSet() {
            const data = await M('saveHeaderSet^MWR', {
                name: this.setName,
                hdr: this.selectedHeaderSet,
                id: this.ID
            })
            ...
            if (this.setName == "") {
                ...
                this.selectedSet = this.setName;
            }

            if (status === 'OK') {
                this.$notify({
                    title: 'Success',
                    message: !this.setName ? 'Selections Loaded' : 'Set Saved',
                    type: 'success'
                })
            }
             this.setLoaded = true;
        ...
        async handleMounted() {
            ...
            const getReportData = async function () {
                const Rdata = await getReportTable(vm.ID, vm.reportTableCurrentRows)
                vm.oHeader = Rdata.rows.oheaders || Rdata.rows.headers ;
                vm.sets = Rdata.sets
                if (Rdata.selectedSet){
                     vm.selectedSet = Rdata.selectedSet;
                     vm.setLoaded = true;
                }
                ...
                if (!vm.setLoaded && vm.$store.state.reports.cachedReports && vm.$store.state.reports.cachedReports[vm.ID]){
                   vm.loading = false
